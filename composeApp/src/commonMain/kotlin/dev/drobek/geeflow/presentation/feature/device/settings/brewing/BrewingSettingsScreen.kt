...
import geeflow.composeapp.generated.resources.common_time
import geeflow.composeapp.generated.resources.device_settings_brewing
import geeflow.composeapp.generated.resources.device_settings_brewing_boiler
...
private fun BoilerSection(
    ...
    VerticalSpacer(24.dp)
    PulseHeating(
        pulseHeatingEnabled = viewState.pulseHeatingEnabled,
        onEvent = onEvent,
        modifier = Modifier.fillMaxWidth()
    )
}
...
private fun PaddleSection(
    ...
    Row {
        Column(
            ...
            Text(
                text = stringResource(Res.string.common_pressure),
...
@Composable
private fun PulseHeating(
    pulseHeatingEnabled: Boolean,
    onEvent: (BrewingSettingsEvent) -> Unit,
    modifier: Modifier = Modifier
) {
    Row(modifier = modifier, verticalAlignment = Alignment.CenterVertically) {
        Column(modifier = Modifier.fillMaxWidth().weight(1f)) {
            Text(
                text = stringResource(Res.string.device_settings_brewing_pulse_heating),
                style = MaterialTheme.typography.titleMedium,
                color = MaterialTheme.colorScheme.onSurface
            )
            VerticalSpacer(4.dp)
            Text(
                text = stringResource(Res.string.device_settings_brewing_pulse_heating_description),
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
        }
        HorizontalSpacer(16.dp)
        GeeFlowSwitch(
            checked = pulseHeatingEnabled,
            onCheckedChange = { onEvent(PulseHeatingToggled(it)) },
            enabled = true,
            modifier = Modifier
        )
    }
