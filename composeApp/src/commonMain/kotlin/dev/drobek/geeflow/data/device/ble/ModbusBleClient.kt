        ...
        mutex.withLock {
            withTimeout(timeoutMs) {
                val ackDeferred = async {
                    bleClient.incomingData.first { (uuid, data) ->
                        uuid.contains(rxUuid) && data.size >= expectedPrefix.size && data.sliceArray(expectedPrefix.indices)
    ...
    suspend fun writeAndAwaitModbus(
        ...
        mutex.withLock {
            withTimeout(timeoutMs) {
                val ackDeferred = async {
                    bleClient.incomingData.first { (uuid, data) ->
                        uuid.contains(rxUuid) && data.size >= 4 &&
                                data[1] == expectedFc &&
